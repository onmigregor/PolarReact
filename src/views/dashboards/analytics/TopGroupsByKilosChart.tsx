...
const CustomTooltip = ({ active, payload }: TooltipProps<number, string>) => {
  if (active && payload && payload.length) {
    const item = payload[0].payload as TopGroupByKilosItem
    ...
    return (
      ...
      >
        ...
        <Typography variant='body2'>
          Total kilos: {item.total_kilos.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg
        </Typography>
      </Box>
    )
  }
...
const TopGroupsByKilosChart = ({ startDate, endDate }: Props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error('Error fetching top groups by kilos:', error)
        setData([])
      } finally {
        setLoading(false)
      }
  ...
  return (
    <Card>
      <CardHeader
        title='📊 Grupos por Kilos'
        subheader={loading ? 'Cargando...' : `${data.length} grupos (unidad: kg)`}
          ...
          <Box sx={{ height: 400 }}>
            <ResponsiveContainer>
              ...
              >
                ...
                <Bar dataKey='total_kilos' radius={[0, 4, 4, 0]} barSize={24}>
...
export default TopGroupsByKilosChart
